        ...
        <div class="w-full">
            <section class="flex flex-col break-words bg-form sm:border-1 sm:rounded-md sm:shadow-sm sm:shadow-lg">
                ...
                <header class="font-semibold bg-header-form text-white text-2xl py-5 px-6 sm:py-6 sm:px-8 sm:rounded-t-md">
                    Edita el género de {{ $genero->genero }}
                </header>

                <form class="w-full px-6 space-y-6 sm:px-10 sm:space-y-8 text-black" method="POST" action="{{ route('generos.update', ['genero' => $genero->id]) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="flex flex-wrap">
                        ...
                        <input placeholder="Introduce el género" class="form-input w-full @error('precio') border-red-500 @enderror" type="text" name="genero" value="{{ old('genero') ?? $genero->genero }}">
                    </div>
